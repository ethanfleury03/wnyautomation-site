...
function markdownToHtml(markdown) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (ctaMatch) {
      closeList();
      html.push(
        `<div id="article-cta" class="article-cta-actions"><a class="button button-primary article-cta-button" href="${escapeAttribute(ctaMatch[2])}" style="color:#10251e;text-decoration:none">${icon("send")}${escapeHtml(ctaMatch[1])}</a></div>`,
      );
      continue;
    }
